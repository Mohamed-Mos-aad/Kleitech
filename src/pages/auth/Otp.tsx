...
// \*\* Api
import { forgetPassword, registerUser, verifyResetCode } from '../../api/auth/authApi';
// \*\* Firebase
...
const sendEmailHandler = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>)=>{
    e.preventDefault();
    if(otpEmail.otpEmail)
    {
        if(otpEmail.purpose === 'resetPassword')
        {
            await forgetPassword({email: otpEmail.otpEmail});
            
        }
        else if(otpEmail.purpose === 'signUp')
        {
...
const userRegisterHandler = async ()=>{
    try{
        ...
        if (res?.token) {
            await createUserWithEmailAndPassword(auth, userData.userEmail, userData.userEmail);
        }
...
const submitCodeHandler = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>)=>{
    e.preventDefault();
    ...
    if(otpEmail.otpEmail)
    {
        ...
        {
            const validCode = await verifyResetCode({email: otpEmail.otpEmail,token:codeInput});
            if(validCode)
            {
                dispatch(setOtpEmail({ otpEmail: otpEmail.otpEmail, purpose: 'resetPassword',token:codeInput}));
                goResetPasswordPage();
            }
        }
        else if(otpEmail.purpose === 'signUp' && otpEmail)
        {
